...
pub struct Config {
    ...
    /// event.
    ///
    /// Example `$CODEX_HOME/config.toml` snippet:
    ///
    /// ```toml
    ...
    /// Directory containing all Codex state. Redou resolves this to the
    /// project-local `.redou/redou-codex` directory unless overridden by
    /// `REDOU_CODEX_HOME` or `CODEX_HOME`.
    ...
    /// Settings that govern if and what will be written to `$CODEX_HOME/history.jsonl`.
    pub history: History,
...
///
/// `REDOU_CODEX_HOME` is preferred, `CODEX_HOME` is retained as an advanced
/// override, and the default is project-local `.redou/redou-codex`.
///
