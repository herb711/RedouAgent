...
fn reject_official_codex_backend_websocket_url(url: &Url) -> Result<(), ApiError> {
    if url
        .as_str()
        .to_ascii_lowercase()
...
async fn connect_websocket(
    ...
    reject_official_codex_backend_websocket_url(&url)?;
...
mod tests {
    ...
    #[test]
    fn rejects_official_codex_backend_websocket_urls() {
        let url = Url::parse("wss://chatgpt.com/backend-api/codex/responses")
            .expect("URL should parse");
        let err = reject_official_codex_backend_websocket_url(&url)
            .expect_err("official Codex websocket backend URL should be rejected");
