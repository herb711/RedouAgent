...
const REDOU_OFFICIAL_CODEX_BACKEND_FRAGMENT: &str = "chatgpt.com/backend-api/codex";
const REDOU_PROVIDER_CONFIG_NOT_APPLIED_ERROR: &str =
    "当前 runtime 仍在连接官方 Codex 后端，provider 配置未生效。";

fn reject_official_codex_backend_realtime_websocket_url(url: &Url) -> Result<(), ApiError> {
    if url
        .as_str()
        .to_ascii_lowercase()
        .contains(REDOU_OFFICIAL_CODEX_BACKEND_FRAGMENT)
    {
        return Err(ApiError::Transport(TransportError::ProviderConfigNotApplied(
            REDOU_PROVIDER_CONFIG_NOT_APPLIED_ERROR.to_string(),
        )));
    }
...
impl RealtimeWebsocketClient {
    ...
    async fn connect_realtime_websocket_url(
        ...
        reject_official_codex_backend_realtime_websocket_url(&ws_url)?;
...
mod tests {
    ...
    #[test]
    fn rejects_official_codex_backend_realtime_websocket_urls() {
        let url = Url::parse("wss://chatgpt.com/backend-api/codex/realtime")
            .expect("URL should parse");
        let err = reject_official_codex_backend_realtime_websocket_url(&url)
            .expect_err("official Codex realtime websocket backend URL should be rejected");

        let ApiError::Transport(TransportError::ProviderConfigNotApplied(message)) = err else {
            panic!("expected ApiError::Transport(ProviderConfigNotApplied)");
        };
        assert_eq!(message, REDOU_PROVIDER_CONFIG_NOT_APPLIED_ERROR);
    }
