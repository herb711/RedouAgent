  ...
  return Effect.gen(function* () {
    const events = Stream.callback<any>((queue) => {
      return Effect.acquireRelease(
        Effect.sync(() => RedouCodexUI.onGlobalEvent((event) => Queue.offerUnsafe(queue, event.payload))),
        (off) => Effect.sync(() => off()),
      )
    })
    const heartbeat = Stream.tick("10 seconds").pipe(
      Stream.drop(1),
      Stream.map(() => ({ id: Bus.createID(), type: "server.heartbeat", properties: { source: "redou-codex" } })),
    )
    log.info("redou-codex instance event connected")
    return HttpServerResponse.stream(
      Stream.make({ id: Bus.createID(), type: "server.connected", properties: { source: "redou-codex" } }).pipe(
        Stream.concat(events.pipe(Stream.merge(heartbeat, { haltStrategy: "left" }))),
        Stream.map(eventData),
        Stream.pipeThroughChannel(Sse.encode()),
        Stream.encodeText,
        Stream.ensuring(Effect.sync(() => log.info("redou-codex instance event disconnected"))),
      ),
      {
        contentType: "text/event-stream",
        headers: {
  ...
  Effect.gen(function* () {
    ...
    return handlers.handleRaw(
      ...
      Effect.fn("EventHttpApi.subscribe")(function* () {
        if (RedouCodexUI.enabled()) return yield* redouEventResponse()
