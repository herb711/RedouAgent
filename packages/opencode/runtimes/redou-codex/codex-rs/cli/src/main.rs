...
#[clap(
    ...
    // `redou-codex-x86_64-unknown-linux-musl`, but the help output should always
    // use the Redou-owned `redou-codex` command name.
    bin_name = "redou-codex",
    override_usage = "redou-codex [OPTIONS] [PROMPT]\n       redou-codex [OPTIONS] <COMMAND> [ARGS]"
)]
struct MultitoolCli {
    #[clap(flatten)]
...
enum Subcommand {
    /// Run redou-codex non-interactively.
    ...
    /// Manage external MCP servers for redou-codex.
    Mcp(McpCli),

    /// Manage redou-codex plugins.
    Plugin(PluginCli),
    ...
    /// Launch the Redou desktop app (opens the app installer if missing).
    #[cfg(any(target_os = "macos", target_os = "windows"))]
...
struct LoginCommand {
    ...
    #[arg(
        long = "with-api-key",
        help = "Read the API key from stdin (e.g. `printenv REDOU_MODEL_API_KEY | redou-codex login --with-api-key`)"
    )]
    with_api_key: bool,

    #[arg(
        long = "with-access-token",
        help = "Read the access token from stdin (e.g. `printenv CODEX_ACCESS_TOKEN | redou-codex login --with-access-token`)"
    )]
...
fn run_update_command() -> anyhow::Result<()> {
    ...
    {
        anyhow::bail!(
            "`redou-codex update` is not available in debug builds. Install a release build of redou-codex to use this command."
        );
    }
    ...
    #[cfg(not(debug_assertions))]
    {
        let Some(action) = codex_tui::get_update_action() else {
            anyhow::bail!(
                "Could not detect the redou-codex installation method. Please rebuild the project-local runtime manually."
            );
        };
        run_update_action(action)
    }
...
async fn cli_main(arg0_paths: Arg0DispatchPaths) -> anyhow::Result<()> {
    ...
    match subcommand {
        ...
        Some(Subcommand::Review(ReviewCommand {
            ...
            let mut exec_cli = ExecCli::try_parse_from(["redou-codex", "exec"])?;
            exec_cli
                .shared
                .inherit_exec_root_options(&interactive.shared);
            ...
            match login_cli.action {
                ...
                None => {
                    if login_cli.with_api_key && login_cli.with_access_token {
                        ...
                        eprintln!(
                            "The --api-key flag is no longer supported. Pipe the key instead, e.g. `printenv REDOU_MODEL_API_KEY | redou-codex login --with-api-key`."
                        );
                        std::process::exit(1);
        ...
        Some(Subcommand::Sandbox(mut sandbox_cli)) => {
            ...
            #[cfg(not(any(target_os = "macos", target_os = "linux", target_os = "windows")))]
            {
                ...
                anyhow::bail!("`redou-codex sandbox` is not supported on this operating system");
            }
        }
...
fn profile_v2_for_subcommand<'a>(
    ...
    match subcommand {
        ...
        _ => anyhow::bail!(
            "--profile only applies to runtime commands and `redou-codex mcp`: `redou-codex`, `redou-codex exec`, `redou-codex review`, `redou-codex resume`, `redou-codex fork`, `redou-codex mcp`, `redou-codex sandbox`, and `redou-codex debug prompt-input`."
        ),
    }
}
...
async fn load_exec_server_remote_auth_provider(
    ...
    let auth = load_exec_server_remote_auth(
        config,
        "remote exec-server registration requires ChatGPT authentication; run `redou-codex login` first",
    )
    .await?;
...
fn reject_remote_mode_for_subcommand(
    ...
    if let Some(remote) = remote {
        anyhow::bail!(
            "`--remote {remote}` is only supported for interactive TUI commands, not `redou-codex {subcommand}`"
        );
    }
    if remote_auth_token_env.is_some() {
        anyhow::bail!(
            "`--remote-auth-token-env` is only supported for interactive TUI commands, not `redou-codex {subcommand}`"
        );
    }
    Ok(())
}
...
/// `redou-codex --strict-config` continue to work for the TUI and for wrappers that
...
fn reject_strict_config_for_unsupported_subcommand(
    ...
    if strict_config {
        anyhow::bail!("`--strict-config` is not supported for `redou-codex {subcommand}`");
    }
    Ok(())
}
...
/// Build the final `TuiCli` for a `redou-codex resume` invocation.
fn finalize_resume_interactive(
    ...
    // configuration surface area as `redou-codex` without additional flags.
...
/// Build the final `TuiCli` for a `redou-codex fork` invocation.
...
/// Merge flags provided to `redou-codex resume`/`redou-codex fork` so they take precedence over any
