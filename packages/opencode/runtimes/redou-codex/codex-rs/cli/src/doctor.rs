//! Implements the `redou-codex doctor` diagnostic report.
//!
...
/// JSON support report emitted by `redou-codex doctor --json`.
///
    ...
    match load_auth_dot_json(&config.codex_home, config.cli_auth_credentials_store_mode) {
        Ok(Some(auth)) => {
            ...
            if status == CheckStatus::Fail {
                check =
                    check.remediation("Run redou-codex login again or provide a supported auth env var.");
            }
            check
        }
        ...
        .details(details)
        .remediation("Run redou-codex login or provide an API key through a supported auth env var."),
        ...
        .detail(err.to_string())
        .remediation("Fix auth storage access or run redou-codex login again."),
    }
}
