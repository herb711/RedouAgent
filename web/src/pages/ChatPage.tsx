...
import type { ChatProject, ChatTask } from "@/lib/api";
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const modelToolsLabel = locale === "zh" ? "项目" : "Projects";
  ...
  const channel = useMemo(
    () => generateChannelId(),
    [resumeParam, selectedProjectId, selectedTaskId],
  );

  const selectTask = useCallback(
    (project: ChatProject, task: ChatTask) => {
      const next = new URLSearchParams(searchParams);
      const nextResume = task.hermesSessionId ?? task.session_id ?? "";
      const same =
        next.get("project") === project.id &&
        next.get("task") === task.id &&
        (next.get("resume") ?? "") === nextResume;
  ...
  useEffect(() => {
    if (!isActive) return;
    const draft = readPendingChatDraft();
    if (!draft) return;
    const ws = wsRef.current;
  ...
  useEffect(() => {
    ...
    ws.onopen = () => {
      ...
      if (pendingDraftRef.current) {
        ws.send(pendingDraftRef.current);
  ...
  }, [channel, resumeParam, selectedProjectId, selectedTaskId]);
  ...
  //   top toolbar — current task model and compact tool-call status.
  //     Best-effort: if that feed fails, the terminal pane
  //     keeps working.
  //
    ...
    createPortal(
      <>
        ...
        <div
          ...
          className={cn(
              "font-mondwest fixed top-0 right-0 z-[60] flex h-dvh max-h-dvh w-80 min-w-0 flex-col antialiased",
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        {!narrow && (
          <div
            ...
            role="complementary"
            aria-label={modelToolsLabel}
            className="flex min-h-0 shrink-0 flex-col overflow-hidden lg:h-full lg:w-[22rem]"
          >
