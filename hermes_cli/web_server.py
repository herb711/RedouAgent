...
def _safe_app_data_file(path_value: Any) -> Path:
    path = Path(str(path_value or "")).expanduser().resolve(strict=False)
    root = get_redou_app_data_root().resolve(strict=False)
    try:
        path.relative_to(root)
    except ValueError:
        raise HTTPException(status_code=400, detail="Context file path is outside app-data")
...
def _project_context_file_path(project: Dict[str, Any], kind: str) -> Path:
    ensure_project_app_data(project)
    if kind == "rules":
        return _safe_app_data_file(project.get("rulesPath"))
    if kind == "memory":
        return _safe_app_data_file(project.get("memoryPath"))
...
def _task_context_file_path(project: Dict[str, Any], task: Dict[str, Any], kind: str) -> Path:
    ensure_task_app_data(project, task)
    if kind == "rules":
        return _safe_app_data_file(task.get("rulesPath"))
    if kind == "summary":
        return _safe_app_data_file(task.get("summaryPath"))
...
def _build_model_setup_catalog() -> List[Dict[str, Any]]:
    ...
    for entry in catalog:
        ...
        entry["api_key_set"] = bool(key and env_on_disk.get(key))
...
def get_model_setup_catalog():
    try:
        ...
        return {"providers": _build_model_setup_catalog(), "current": current}
    except Exception:
...
async def setup_main_model(body: ModelSetup):
    ...
    try:
        ...
        is_builtin_provider = provider in PROVIDER_REGISTRY
    except Exception:
        is_builtin_provider = provider in {
            "deepseek",
            "minimax",
            "minimax-cn",
            "kimi-coding",
