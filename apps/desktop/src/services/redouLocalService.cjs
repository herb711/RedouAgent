...
class RedouLocalService {
  ...
  readAllProjects() {
    ...
    for (const entry of fs.readdirSync(this.projectsDir(), { withFileTypes: true })) {
      ...
      if (project && typeof project === "object") {
        projects.push(this.ensureProject(project));
      }
    }
  ...
  runDashboardBridge(action, payload = {}) {
    ...
    const result = spawnSync(this.pythonPath, [bridgePath, action], {
  ...
  runAnalysisTaskProcess({
    ...
    return new Promise((resolve, reject) => {
      ...
      const child = spawn(this.pythonPath, [adapterPath], {
        cwd: workspacePath,
        env: this.childEnv({
          HERMES_HOME: this.hermesHome,
          REDOU_APP_DATA_ROOT: this.appDataRoot(),
  ...
  runContextCompactModel(payload, project) {
    ...
    const result = spawnSync(this.pythonPath, [compactorPath], {
  ...
  stopRun(runId, webContents) {
    ...
    this.emitToRenderer(webContents, {
      runId,
      projectId: run.projectId,
      taskId: run.taskId,
      event,
