...
class HermesCLI:
    ...
    def __init__(
        ...
        # busy_input_mode: "queue" (default; Enter queues for next turn),
        # "steer" (Enter injects mid-run via /steer, arriving after the
        # next tool call), or "interrupt" (Enter interrupts current run).
        self.busy_input_mode = normalize_busy_input_mode(
            CLI_CONFIG["display"].get("busy_input_mode", DEFAULT_BUSY_INPUT_MODE)
        )
