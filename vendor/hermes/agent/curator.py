...
CURATOR_REVIEW_PROMPT = (
    ...
    "salvage` for the PR review cluster). Prefer `skill_manage "
    "action=merge` when the siblings can be absorbed as labeled sections; "
    "use patch/write_file first when you need to hand-shape the umbrella.\n"
    ...
    "  - skill_manage action=merge      — absorb source skills into an "
    "existing umbrella and archive the sources\n"
    "  - skill_manage action=patch      — add or refine sections in the umbrella\n"
...
def _classify_removed_skills(
    ...
    for name in removed:
        ...
        for args in parsed_calls:
            ...
            if args.get("action") == "merge" and isinstance(merge_sources, list):
                if name in {str(item) for item in merge_sources}:
                    into = target
                    evidence = (
                        f"skill_manage action=merge on '{target}' "
                        f"listed '{name}' in merge_sources"
                    )
                    break
