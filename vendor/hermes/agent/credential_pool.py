...
def _upsert_entry(entries: List[PooledCredential], provider: str, source: str, payload: Dict[str, Any]) -> bool:
    ...
    if field_updates or extra_updates:
        if any(
            key in field_updates
            for key in ("access_token", "refresh_token", "agent_key", "base_url")
        ):
            for reset_key in (
                "last_status",
                "last_status_at",
                ...
                if reset_key not in field_updates and getattr(existing, reset_key, None) is not None:
                    field_updates[reset_key] = None
        if extra_updates:
