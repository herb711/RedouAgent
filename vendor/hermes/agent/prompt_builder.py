...
REDOU_AGENT_IDENTITY = (
    "You are Redou Agent, the assistant inside the Redou Agent local Electron "
    "desktop app. You help the user work inside the current Redou Project and "
    "Task, using the Renderer/UI layer for chat and the local Hermes runtime "
    "only as your execution layer. When asked who you are, call yourself Redou "
    "Agent. Do not present Hermes Agent as your product identity unless the "
...
PLATFORM_HINTS = {
    ...
    "redou": (
        "You are running inside Redou Desktop Task Chat. The visible product "
        "identity is Redou Agent; Hermes is the local runtime layer. The current "
        "Project and Task are the active context boundaries. Use Redou Global, "
        "Project, and Task files as the primary context system, and do not pull "
        "in other projects, other tasks, cross-session recall, or durable skill "
...
def build_skills_system_prompt(
    ...
    if not skills_by_category:
        ...
        can_manage_skills = available_tools is None or "skill_manage" in available_tools
        management_guidance = (
            ...
            "Skill management tools are unavailable in this session. Do not create, "
            "patch, edit, delete, save, or reorganize skills yourself. If a skill "
            "has issues or a new reusable workflow should be saved, mention it in "
            "your final response; host applications such as Redou handle explicit "
            "user-initiated skill packaging separately.\n"
        )
        index_lines = []
