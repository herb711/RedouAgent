...
By default, messaging a busy agent queues the new message for the next turn. Two other modes are available:

- `queue` — follow-up messages wait and run as the next turn after the current task finishes. This is the default.
