...
Actions:
  ...
  merge      -- Absorb one or more skills into a target skill and archive sources
...
def _safe_merge_slug(value: str, fallback: str = "skill") -> str:
    slug = re.sub(r"[^A-Za-z0-9._-]+", "-", str(value or "").strip().lower())
    slug = re.sub(r"^[._-]+|[._-]+$", "", slug)[:80]
    return slug or fallback


def _split_skill_markdown(text: str) -> Tuple[str, str]:
    ...
    if close < 0:
        return "", normalized
    after = normalized.find("\n", close + 4)
    if after < 0:
        return normalized, ""
    return normalized[: after + 1], normalized[after + 1 :].lstrip("\n")


def _skill_description_from_text(text: str) -> str:
    frontmatter, _body = _split_skill_markdown(text)
    if not frontmatter:
        return ""
    try:
...
def _copy_merge_support_files(target_dir: Path, source_dir: Path, slug: str) -> Tuple[List[str], List[Path]]:
    copied: List[str] = []
    copied_dirs: List[Path] = []
    for folder in sorted(ALLOWED_SUBDIRS):
        source_folder = source_dir / folder
        if not source_folder.is_dir():
            continue
        destination = target_dir / folder / "merged" / slug
        counter = 2
        while destination.exists():
            destination = target_dir / folder / "merged" / f"{slug}-{counter}"
            counter += 1
        shutil.copytree(source_folder, destination)
        copied_dirs.append(destination)
        copied.append(str(destination.relative_to(target_dir)))
    return copied, copied_dirs


def _archive_merged_skill(source_dir: Path, skills_root: Path, slug: str, timestamp: str) -> Path:
    archive_root = skills_root / ".archive"
    archive_root.mkdir(parents=True, exist_ok=True)
    destination = archive_root / f"merged-{slug}-{timestamp}"
    counter = 2
    while destination.exists():
        destination = archive_root / f"merged-{slug}-{timestamp}-{counter}"
        counter += 1
    shutil.move(str(source_dir), str(destination))

    parent = source_dir.parent
    while parent != skills_root and parent.exists() and not any(parent.iterdir()):
...
def _merge_skill(name: str, merge_sources: List[str]) -> Dict[str, Any]:
    """Absorb one or more source skills into a target skill.

    The target SKILL.md is rewritten once with labeled sections appended.
    Source support files are copied under the target's support directories,
    and source skill directories are moved to ``.archive`` after the target
    update passes validation and security scanning.
    """
    err = _validate_name(name)
    if err:
        return {"success": False, "error": err}
    ...
    if not isinstance(merge_sources, list) or not merge_sources:
        return {
            "success": False,
            "error": "merge_sources must list at least one source skill name.",
        }

    target = _find_skill(name)
    if not target:
        return {"success": False, "error": f"Target skill '{name}' not found."}

    normalized_sources: List[str] = []
    seen = {name}
    for raw in merge_sources:
        source_name = str(raw or "").strip()
        err = _validate_name(source_name)
        if err:
            return {"success": False, "error": f"Invalid source skill '{source_name}': {err}"}
        if source_name in seen:
            return {
                "success": False,
                "error": f"Source skill '{source_name}' cannot be merged into itself or listed twice.",
            }
    ...
    if not target_md.is_file():
        return {"success": False, "error": f"Target skill '{name}' is missing SKILL.md."}

    target_original = target_md.read_text(encoding="utf-8")
    target_text = target_original.rstrip()
    now = datetime.now(timezone.utc)
    timestamp = now.strftime("%Y%m%d%H%M%S")
    copied_dirs: List[Path] = []
    copied_files: List[Dict[str, Any]] = []
    archive_jobs: List[Tuple[str, Path, Path, str]] = []
    resolved_sources: List[Dict[str, Any]] = []

    for source_name in normalized_sources:
        pinned_err = _pinned_guard(source_name)
        if pinned_err:
            return {"success": False, "error": pinned_err}

        source = _find_skill(source_name)
        if not source:
            return {"success": False, "error": f"Source skill '{source_name}' not found."}
        source_dir = source["path"]
        if source_dir == target_dir:
            return {"success": False, "error": f"Source skill '{source_name}' cannot equal target skill '{name}'."}
        source_md = source_dir / "SKILL.md"
        if not source_md.is_file():
            return {"success": False, "error": f"Source skill '{source_name}' is missing SKILL.md."}

        source_text = source_md.read_text(encoding="utf-8")
        _frontmatter, source_body = _split_skill_markdown(source_text)
        resolved_sources.append(
            {
                "name": source_name,
                "dir": source_dir,
                "body": source_body,
                "description": _skill_description_from_text(source_text),
    ...
    for source in resolved_sources:
        ...
        slug = f"{_safe_merge_slug(source_name)}-{timestamp}"
        copied, new_dirs = _copy_merge_support_files(target_dir, source_dir, slug)
        copied_dirs.extend(new_dirs)
        copied_files.append({"name": source_name, "paths": copied})
        support_line = (
            f"- Supporting files copied under: {', '.join(f'`{item}`' for item in copied)}"
            if copied
            else "- Supporting files copied under: (none)"
        )
        section = [
            "",
            "",
            f"## Merged Skill: {source_name}",
            "",
            f"- Source skill: `{source_name}`",
            f"- Merged at: {now.isoformat(timespec='seconds').replace('+00:00', 'Z')}",
            support_line,
        ...
        archive_jobs.append((source_name, source_dir, source["skills_root"], slug))

    merged_content = f"{target_text.rstrip()}\n"
    err = _validate_frontmatter(merged_content)
    if err:
        for copied_dir in copied_dirs:
            shutil.rmtree(copied_dir, ignore_errors=True)
        return {"success": False, "error": f"Merge would break SKILL.md structure: {err}"}
    err = _validate_content_size(merged_content)
    if err:
        for copied_dir in copied_dirs:
            shutil.rmtree(copied_dir, ignore_errors=True)
        return {"success": False, "error": err}

    _atomic_write_text(target_md, merged_content)
    scan_error = _security_scan_skill(target_dir)
    if scan_error:
        _atomic_write_text(target_md, target_original)
        for copied_dir in copied_dirs:
            shutil.rmtree(copied_dir, ignore_errors=True)
        return {"success": False, "error": scan_error}
    ...
    try:
        for source_name, source_dir, skills_root, slug in archive_jobs:
            archived_path = _archive_merged_skill(source_dir, skills_root, slug, timestamp)
            moved_sources.append((archived_path, source_dir))
            archived.append({"name": source_name, "path": str(archived_path)})
    except Exception:
        _atomic_write_text(target_md, target_original)
        for copied_dir in copied_dirs:
            shutil.rmtree(copied_dir, ignore_errors=True)
        for archived_path, original_dir in reversed(moved_sources):
            try:
                ...
                logger.debug("Failed to restore archived source %s", archived_path, exc_info=True)
        raise

    return {
        "success": True,
        "message": (
            f"Merged {len(archived)} skill"
            f"{'s' if len(archived) != 1 else ''} into '{name}'."
        ),
        "merged_into": name,
        "path": str(target_dir),
        "skill_md": str(target_md),
        "archived": archived,
...
def skill_manage(
    ...
    if action == "create":
        ...
        result = _merge_skill(name, merge_sources or [])
        ...
        result = {"success": False, "error": f"Unknown action '{action}'. Use: create, edit, patch, merge, delete, write_file, remove_file"}
    ...
    if result.get("success"):
        ...
        try:
            ...
            if action == "create":
                ...
                bump_patch(name)
                for source_name in merge_sources or []:
                    forget(str(source_name))
...
SKILL_MANAGE_SCHEMA = {
    "name": "skill_manage",
    "description": (
        "Manage skills (create, update, merge, delete). Skills are your procedural "
        "memory — reusable approaches for recurring task types. "
        f"New skills go to {display_hermes_home()}/skills/; existing skills can be modified wherever they live.\n\n"
        "Actions: create (full SKILL.md + optional category), "
        "patch (old_string/new_string — preferred for fixes), "
        "edit (full SKILL.md rewrite — major overhauls only), "
        "merge (absorb source skills into a target and archive sources), "
        "delete, write_file, remove_file.\n\n"
        "For deterministic consolidation, prefer merge: choose an existing "
        "target skill as `name` and pass source names in `merge_sources`. "
        "On delete, pass `absorbed_into=<umbrella>` only when content was "
        "already merged by some other path, or `absorbed_into=\"\"` when "
        ...
        "Pinned skills are protected from destructive source removal — delete "
        "and merge-source archival refuse with a message pointing the user to "
        "`hermes curator unpin <name>`. Patches and edits go through on pinned "
        "skills so you can still improve them as pitfalls come up; pin only "
        "guards against irrecoverable loss."
    ),
    "parameters": {
        "type": "object",
        "properties": {
            "action": {
                "type": "string",
                "enum": ["create", "patch", "edit", "merge", "delete", "write_file", "remove_file"],
            ...
            "name": {
                ...
                "description": (
                    ...
                    "Must match an existing skill for patch/edit/merge/delete/write_file/remove_file."
                )
            },
            "content": {
                "type": "string",
            ...
            "merge_sources": {
                ...
                "items": {"type": "string"},
                "description": (
                    "For 'merge' only. Source skill names to absorb into the "
                    "target named by `name`. The target SKILL.md is appended "
                    "with labeled source sections, source support files are "
                    "copied under merged/ folders, and source directories are "
                    "moved to .archive after validation."
                )
            },
            "absorbed_into": {
                "type": "string",
...
registry.register(
    ...
    handler=lambda args, **kw: skill_manage(
        ...
        merge_sources=args.get("merge_sources"),
