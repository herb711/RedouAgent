...
def _windows_shell_path_to_native(path: str) -> str:
    """Translate MSYS/Cygwin/WSL drive paths to native Windows paths.

    The local backend runs under native Windows Python, but the bash it drives
    can report cwd values such as ``/c/Users/...`` or ``/mnt/c/Users/...``.
    ``subprocess.Popen(cwd=...)`` and ``os.path.isdir`` need ``C:\\Users\\...``.
    """
    ...
    rest_native = rest.replace("/", "\\")
    return f"{drive.upper()}:\\{rest_native}"


def _host_isdir(path: str) -> bool:
    if not path:
        return False
    if os.path.isdir(path):
        return True
    native = _windows_shell_path_to_native(path)
...
def _resolve_safe_cwd(cwd: str) -> str:
    ...
    """
    if cwd and _host_isdir(cwd):
        return cwd
    parent = os.path.dirname(cwd) if cwd else ""
    while parent:
        if _host_isdir(parent):
            return parent
...
def _find_bash() -> str:
    ...
    def _is_wsl_launcher(candidate: str) -> bool:
        normalized = os.path.normcase(os.path.normpath(candidate or ""))
        return normalized.endswith(os.path.normcase(r"\Windows\System32\bash.exe")) or normalized.endswith(
            os.path.normcase(r"\Microsoft\WindowsApps\bash.exe")
        )
...
class LocalEnvironment(BaseEnvironment):
    ...
    def _run_bash(self, cmd_string: str, *, login: bool = False,
        ...
        # On Windows, self.cwd may be a shell-style path (/c/Users/...,
        # /cygdrive/c/..., or /mnt/c/...) from bash pwd output. Popen needs a
        # native Windows cwd even when bash itself understands the shell path.
        _popen_cwd = self.cwd
        if _IS_WINDOWS and _popen_cwd:
            _popen_cwd = _windows_shell_path_to_native(_popen_cwd)
    ...
    def _update_cwd(self, result: dict):
        ...
        try:
            ...
            if cwd_path and _host_isdir(cwd_path):
                self.cwd = cwd_path
