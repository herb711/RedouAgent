...
export default function ConfigPage() {
  ...
  const languageCopy =
    locale === "zh"
      ? {
          title: "界面语言",
          description: "切换桌面应用界面语言，选择会写入配置。",
          current: "当前语言",
        }
      : {
          title: "UI Language",
          description: "Switch the desktop app language. The choice is saved to config.",
          current: "Current language",
        };
  const languageOptions = Object.entries(LOCALE_META) as Array<
  ...
  const handleLanguageChange = (code: Locale) => {
    setLocale(code);
    setConfig((prev) =>
      prev ? setNestedValue(prev, "dashboard.language", code) : prev,
    );
  };
          ...
          <CardContent className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex min-w-0 items-start gap-3">
              ...
              <div className="min-w-0">
                <div className="text-sm font-medium">{languageCopy.title}</div>
            ...
            <div
              role="radiogroup"
              aria-label={languageCopy.current}
              className="inline-flex shrink-0 overflow-hidden rounded-md border border-border bg-background/35"
            >
              {languageOptions.map(([code, meta]) => {
                ...
                return (
                  <button
                    ...
                    className={`min-w-24 px-3 py-2 text-xs transition-colors ${
                      active
