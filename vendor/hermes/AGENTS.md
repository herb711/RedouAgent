...
### Redou Desktop Chat Architecture

Redou Agent is a local Electron desktop application. Do not model it as a split
frontend plus independent web service, and do not add a standalone Web backend,
HTTP API, FastAPI app, Express server, or remote service for Redou UI features.

Use these names in Redou prompts, plans, comments, and docs:
...
- **Electron Main Process / Local Service layer**: persistence, app-data
  ...
  management, Hermes CLI child processes, stdout/stderr parsing, and streaming
  structured events to the Renderer through IPC.
- **Local Runtime layer**: Hermes running locally in the background.
...
Renderer code must not call `child_process`, operate the Hermes CLI directly, or
perform heavy local file I/O. It should call IPC handlers exposed by the preload
bridge. Hermes terminal output is never the main UI surface; parse it into
structured `AgentEvent` objects and render a ChatGPT-like task chat with event
cards. Do not turn the UI into a PTY, xterm, or raw terminal relay.
