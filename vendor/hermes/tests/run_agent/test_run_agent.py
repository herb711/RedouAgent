...
from agent.prompt_builder import DEFAULT_AGENT_IDENTITY, REDOU_AGENT_IDENTITY
...
def test_redou_platform_uses_redou_identity_without_hermes_skill_hint():
    with (
        patch(
            "run_agent.get_tool_definitions",
            return_value=_make_tool_defs("web_search"),
    ...
    assert REDOU_AGENT_IDENTITY in prompt
    assert DEFAULT_AGENT_IDENTITY not in prompt
    assert "created by Nous Research" not in prompt
