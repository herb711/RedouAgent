...
def test_openrouter_models_api_pricing_is_converted_from_per_token_to_per_million(monkeypatch):
    monkeypatch.setattr("agent.usage_pricing._lookup_persisted_pricing", lambda route: None)
    monkeypatch.setattr("agent.usage_pricing._cache_pricing_entry", lambda *args, **kwargs: None)
    monkeypatch.setattr(
...
def test_estimate_usage_cost_uses_official_provider_pricing_for_redou_models(monkeypatch):
    monkeypatch.setattr("agent.usage_pricing._lookup_persisted_pricing", lambda route: None)
    monkeypatch.setattr("agent.usage_pricing._cache_pricing_entry", lambda *args, **kwargs: None)

    result = estimate_usage_cost(
        "kimi-k2.6",
        CanonicalUsage(input_tokens=1_000_000, output_tokens=1_000_000, cache_read_tokens=1_000_000),
        provider="kimi-coding-cn",
    )

    assert result.status == "estimated"
    assert result.source == "official_docs_snapshot"
    assert float(result.amount_usd) == 5.11


def test_estimate_usage_cost_supports_xiaomi_mimo_pricing(monkeypatch):
    monkeypatch.setattr("agent.usage_pricing._lookup_persisted_pricing", lambda route: None)
    monkeypatch.setattr("agent.usage_pricing._cache_pricing_entry", lambda *args, **kwargs: None)
    ...
    assert result.status == "estimated"
    assert result.source == "official_docs_snapshot"
    assert float(result.amount_usd) == 4.2


def test_estimate_usage_cost_infers_vendor_prefixes(monkeypatch):
    monkeypatch.setattr("agent.usage_pricing._lookup_persisted_pricing", lambda route: None)
    monkeypatch.setattr("agent.usage_pricing._cache_pricing_entry", lambda *args, **kwargs: None)
    ...
    assert result.status == "estimated"
    assert float(result.amount_usd) == 5.8
