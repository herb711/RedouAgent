...
class TestBuildSkillsSystemPromptConditional:
    ...
    def test_read_only_skills_prompt_omits_skill_manage_guidance(self, monkeypatch, tmp_path):
        monkeypatch.setenv("HERMES_HOME", str(tmp_path))
        skill_dir = tmp_path / "skills" / "redou-tasks" / "server-check"
        skill_dir.mkdir(parents=True)
        (skill_dir / "SKILL.md").write_text(
            "---\nname: server-check\ndescription: Check a server\n---\n"
        )
        result = build_skills_system_prompt(
            available_tools={"skills_list", "skill_view"},
            available_toolsets={"skills"},
        )
        assert "server-check" in result
        assert "skill_manage" not in result
        assert "Skill management tools are unavailable" in result
