...
class TestResolveSafeCwd:
    ...
    def test_windows_shell_path_exists_via_native_path(self, monkeypatch):
        ...
        monkeypatch.setattr(local_mod.os.path, "isdir", fake_isdir)

        assert (
            _resolve_safe_cwd("/mnt/c/Users/Administrator/workspace")
            == "/mnt/c/Users/Administrator/workspace"
        )
        assert (
            _resolve_safe_cwd("/mnt/c/Users/Administrator/workspace/deleted")
            == "/mnt/c/Users/Administrator/workspace"
        )


class TestWindowsShellPathToNative:
    def test_translates_msys_cygwin_and_wsl_drive_paths(self, monkeypatch):
        import tools.environments.local as local_mod

        monkeypatch.setattr(local_mod, "_IS_WINDOWS", True)

        assert _windows_shell_path_to_native("/c/Users/foo") == r"C:\Users\foo"
        assert _windows_shell_path_to_native("/cygdrive/d/data") == r"D:\data"
        assert _windows_shell_path_to_native("/mnt/c/Users/foo") == r"C:\Users\foo"
        assert _windows_shell_path_to_native(r"C:\Users\foo") == r"C:\Users\foo"
    ...
    def test_posix_noop(self, monkeypatch):
        ...
        assert _windows_shell_path_to_native("/mnt/c/Users/foo") == "/mnt/c/Users/foo"


class TestFindBashOnWindows:
    def test_prefers_git_bash_over_wsl_launcher(self, monkeypatch):
        import tools.environments.local as local_mod
    ...
    def test_rejects_wsl_launcher_when_git_bash_missing(self, monkeypatch):
        import tools.environments.local as local_mod
        ...
        try:
            ...
            assert "Git Bash not found" in str(exc)
        else:
            raise AssertionError("WSL bash launcher should not be accepted as Git Bash")
...
class TestRunBashCwdRecovery:
    ...
    def test_windows_popen_receives_native_cwd_for_wsl_marker_path(self, monkeypatch):
        import tools.environments.local as local_mod

        shell_cwd = "/mnt/c/Users/Administrator/workspace"
        with patch.object(LocalEnvironment, "init_session", autospec=True, return_value=None):
            env = LocalEnvironment(cwd=shell_cwd, timeout=10)

        captured = {}
        fds: list = []
        try:
            monkeypatch.setattr(local_mod, "_IS_WINDOWS", True)
            with patch("tools.environments.local._find_bash", return_value=r"C:\Program Files\Git\bin\bash.exe"), \
                 patch("tools.environments.local._resolve_safe_cwd", return_value=shell_cwd), \
                 patch("subprocess.Popen", side_effect=_make_fake_popen(captured, fds)), \
                 patch("tools.terminal_tool._interrupt_event", _fake_interrupt()):
                env.execute("echo hello")
        finally:
        ...
        assert captured["cwd"] == r"C:\Users\Administrator\workspace"
