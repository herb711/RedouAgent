...
def _skill_content(name, description="A test skill for unit testing.", body="Step 1: Do the thing."):
    return f"""\
---
name: {name}
...
class TestMergeSkill:
    def test_merge_absorbs_source_and_archives_it(self, tmp_path):
        with _skill_dir(tmp_path):
            skill_manage(
                action="create",
                name="umbrella",
                content=_skill_content("umbrella", body="Use the umbrella flow."),
            )
            skill_manage(
                action="create",
                name="narrow",
            ...
            raw = skill_manage(action="merge", name="umbrella", merge_sources=["narrow"])

        result = json.loads(raw)
        assert result["success"] is True, result
        assert result["merged_into"] == "umbrella"
        assert result["archived"][0]["name"] == "narrow"
        assert not (tmp_path / "narrow").exists()
        assert (tmp_path / ".archive").is_dir()
        merged_text = (tmp_path / "umbrella" / "SKILL.md").read_text(encoding="utf-8")
        assert "## Merged Skill: narrow" in merged_text
        assert "Use the narrow flow." in merged_text
        assert (tmp_path / "umbrella" / "references" / "merged").is_dir()

    def test_merge_rejects_self_source(self, tmp_path):
        with _skill_dir(tmp_path):
            skill_manage(
                action="create",
                name="umbrella",
            ...
            raw = skill_manage(action="merge", name="umbrella", merge_sources=["umbrella"])

        result = json.loads(raw)
        assert result["success"] is False
        assert "cannot be merged into itself" in result["error"]
        assert (tmp_path / "umbrella").exists()
