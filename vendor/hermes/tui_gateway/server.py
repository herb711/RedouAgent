...
from hermes_cli.busy_input import DEFAULT_BUSY_INPUT_MODE, normalize_busy_input_mode
...
def _load_busy_input_mode() -> str:
    ...
    if not isinstance(display, dict):
        display = {}
    raw = display.get("busy_input_mode", DEFAULT_BUSY_INPUT_MODE)
...
def _redou_task_ids(params: Optional[dict] = None) -> tuple[str, str]:
    params = params or {}
    project_id = str(
        params.get("project_id") or os.environ.get("REDOU_PROJECT_ID") or ""
    ).strip()
    task_id = str(params.get("task_id") or os.environ.get("REDOU_TASK_ID") or "").strip()
    return project_id, task_id


def _redou_task_messages_payload(params: Optional[dict] = None) -> dict:
    project_id, task_id = _redou_task_ids(params)
    if not project_id or not task_id:
        return {"active": False, "messages": [], "warnings": []}
    try:
        from hermes_cli.redou_context import (
            load_task_messages,
            task_messages_to_transcript_rows,
        )

        loaded = load_task_messages(project_id, task_id)
        messages = task_messages_to_transcript_rows(loaded.get("messages", []))
        payload = {
            "active": True,
            "project_id": project_id,
            "task_id": task_id,
            "messages_path": loaded.get("messagesPath", ""),
            "hermes_session_id": loaded.get("hermesSessionId", ""),
            "messages": messages,
            "raw_messages": loaded.get("messages", []),
            "warnings": loaded.get("warnings", []),
        }
        logger.debug(
            "redou task hydrate projectId=%s taskId=%s messagesPath=%s loadedMessages=%s hermesSessionId=%s",
            project_id,
            task_id,
            payload["messages_path"],
            len(messages),
        ...
        logger.warning(
            "redou task hydrate failed projectId=%s taskId=%s: %s",
            project_id,
            task_id,
            exc,
        )
        return {
            ...
            "task_id": task_id,
            "messages": [],
            "warnings": [f"Failed to load task messages: {type(exc).__name__}"],
        }
...
def _is_redou_runtime() -> bool:
    try:
        from hermes_cli.redou_runtime import is_redou_task_runtime

        return is_redou_task_runtime()
    except Exception:
        return bool(
            os.environ.get("REDOU_PROJECT_ID", "").strip()
            and os.environ.get("REDOU_TASK_ID", "").strip()
        )


def _load_disabled_toolsets() -> list[str] | None:
    if not _is_redou_runtime():
        return None
    try:
        from hermes_cli.redou_runtime import redou_disabled_toolsets

        return redou_disabled_toolsets()
    except Exception:
        return ["memory", "session_search"]


def _load_disabled_tools() -> list[str] | None:
    if not _is_redou_runtime():
        return None
    try:
        from hermes_cli.redou_runtime import redou_disabled_tools

        return redou_disabled_tools()
    except Exception:
        return ["skill_manage"]
...
def _session_info(agent) -> dict:
    ...
    info: dict = {
        ...
        "disabled_tools": getattr(agent, "disabled_tools", None) or [],
    }
    try:
...
def _background_agent_kwargs(agent, task_id: str) -> dict:
    ...
    return {
        ...
        "platform": "redou" if getattr(agent, "platform", "") == "redou" else "tui",
        "session_db": _get_db(),
...
def _make_agent(sid: str, key: str, session_id: str | None = None):
    ...
    return AIAgent(
        ...
        platform="redou" if redou_runtime else "tui",
        session_id=session_id or key,
        session_db=_get_db(),
        ephemeral_system_prompt=system_prompt or None,
        checkpoints_enabled=is_truthy_value(os.environ.get("HERMES_TUI_CHECKPOINTS")),
        pass_session_id=is_truthy_value(os.environ.get("HERMES_TUI_PASS_SESSION_ID")),
        skip_context_files=redou_runtime
        or is_truthy_value(os.environ.get("HERMES_IGNORE_RULES")),
        skip_memory=redou_runtime
        or is_truthy_value(os.environ.get("HERMES_IGNORE_RULES")),
        **_agent_cbs(sid),
    )
...
@method("redou.task.messages")
def _(rid, params: dict) -> dict:
    return _ok(rid, _redou_task_messages_payload(params))
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        redou_messages = _redou_task_messages_payload(params)
        if redou_messages.get("active"):
            messages = list(redou_messages.get("messages") or [])
...
def _run_prompt_submit(rid, sid: str, session: dict, text: Any) -> None:
    ...
    def run():
        ...
        try:
            ...
            redou_project_id = os.environ.get("REDOU_PROJECT_ID", "").strip()
            redou_task_id = os.environ.get("REDOU_TASK_ID", "").strip()
            redou_raw_user_input = text if isinstance(text, str) else str(text)
            if redou_project_id and redou_task_id and isinstance(prompt, str):
                try:
                    from hermes_cli.redou_context import (
                        append_task_message,
                        build_task_context,
                    ...
                    built_context = build_task_context(
                        redou_project_id,
                        redou_task_id,
                        prompt,
                    )
                    prompt = built_context.text
                    append_task_message(
                        redou_project_id,
                        redou_task_id,
                        "user",
                        redou_raw_user_input,
                        hermes_session_id=session.get("session_key") or sid,
                    )
                except Exception as ctx_exc:
                    logger.warning(
                        "redou context build failed projectId=%s taskId=%s profile=%s: %s",
                        redou_project_id,
                        redou_task_id,
                        os.environ.get("REDOU_HERMES_PROFILE", ""),
                        ctx_exc,
                    )
                    prompt = (
                        "# Redou Task Context\n\n"
                        "## Context Warning\n\n"
                        "Context builder failed; only the current user request is available.\n\n"
            ...
            if redou_project_id and redou_task_id:
                try:
                    from hermes_cli.redou_context import (
                        append_task_message,
                        update_task_summary_after_turn,
                    )
                    ...
                    logger.warning(
                        "redou messages append failed projectId=%s taskId=%s role=assistant: %s",
                        redou_project_id,
                        redou_task_id,
                        msg_exc,
                    )
...
def _(rid, params: dict) -> dict:
    try:
        ...
        disabled = (
            set(getattr(session["agent"], "disabled_toolsets", []) or [])
            if session
            else set(_load_disabled_toolsets() or [])
        )
        ...
        for name in sorted(get_all_toolsets().keys()):
            ...
            if not info:
                continue
            is_enabled = name in enabled if enabled else True
            if name in disabled:
                is_enabled = False
            items.append(
                {
...
def _(rid, params: dict) -> dict:
    try:
        ...
        disabled_tools = (
            getattr(session["agent"], "disabled_tools", None)
            if session
            else _load_disabled_tools()
        )
