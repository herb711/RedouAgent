...
from hermes_cli.redou_context import (
    REDOU_APP_DATA_ENV,
    REDOU_CHAT_PROJECTS_FILE_ENV,
    ensure_global_app_data,
    ensure_project_app_data,
    ensure_project_hermes_profile,
    ensure_task_app_data,
    get_redou_app_data_root,
    load_task_messages,
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "dashboard.language": {
        "type": "select",
        "description": "Web dashboard UI language",
        "options": ["zh", "en"],
    },
...
class ModelSetup(BaseModel):
    """Payload for POST /api/model/setup.

    Saves the selected main model plus optional endpoint/key settings.
    """
    provider: str
    model: str
    base_url: str = ""
...
class ModelSetupRefresh(BaseModel):
    """Payload for POST /api/model/setup/refresh.

    Saves credentials/endpoint first, then probes the provider model list.
    """
    provider: str
    model: str = ""
    base_url: str = ""
...
# Chat projects/tasks
#
# Desktop chat needs a lightweight project layer above Hermes sessions:
# a project owns an isolated workspace directory, and each task points at the
# Hermes session that backs its chat.  The data is intentionally stored outside
# the source tree under HERMES_HOME so copied/renamed app builds do not mix
# user workspaces into the application files.
# ---------------------------------------------------------------------------

_CHAT_PROJECTS_VERSION = 2
_CHAT_PROJECTS_FILE = Path(get_hermes_home()) / "chat-projects.json"
...
class ChatContextFilePatch(BaseModel):
    content: str = ""
...
def _compact_label(value: Optional[str], fallback: str) -> str:
    clean = " ".join(str(value or "").split())
    return clean[:120] if clean else fallback


def _safe_workspace_leaf(name: str) -> str:
    import re

    leaf = re.sub(r"[^A-Za-z0-9._-]+", "-", name.strip()).strip(".-")
    return leaf[:48] or "project"


def _default_workspace_path(name: str) -> Path:
    return Path(get_hermes_home()) / "workspaces" / _safe_workspace_leaf(name)


def _normalise_workspace_path(raw: Optional[str], name: str) -> str:
    try:
        path = Path(raw).expanduser() if raw and raw.strip() else _default_workspace_path(name)
        ...
        raise HTTPException(status_code=400, detail=f"Invalid workspace path: {exc}")


def _compact_optional(value: Optional[str], max_len: int = 160) -> str:
...
def _new_chat_task(
    ...
    now = _chat_now()
    return {
        "id": f"tsk_{secrets.token_hex(6)}",
        "title": _compact_label(title, "New Chat"),
        "session_id": None,
        "model_provider": _compact_optional(model_provider),
        "model": _compact_optional(model),
        "created_at": now,
        "updated_at": now,
    }


def _new_chat_project(
    name: str = "Default Project",
    workspace_path: Optional[str] = None,
) -> Dict[str, Any]:
    now = _chat_now()
    label = _compact_label(name, "Default Project")
    task = _new_chat_task("New Chat")
    project = {
        "id": f"prj_{secrets.token_hex(6)}",
        "name": label,
        "workspace_path": _normalise_workspace_path(workspace_path, label),
        "created_at": now,
        "updated_at": now,
        "tasks": [task],
    }
    ensure_project_hermes_profile(project)
    ensure_project_app_data(project)
    ensure_task_app_data(project, task)
    return project


def _read_chat_projects_store() -> Dict[str, Any]:
    if not _CHAT_PROJECTS_FILE.exists():
        return {"version": _CHAT_PROJECTS_VERSION, "projects": []}
    try:
        raw = json.loads(_CHAT_PROJECTS_FILE.read_text(encoding="utf-8"))
        return raw if isinstance(raw, dict) else {"version": _CHAT_PROJECTS_VERSION, "projects": []}
    except Exception:
        _log.exception("Failed to read chat projects store")
        return {"version": _CHAT_PROJECTS_VERSION, "projects": []}


def _write_chat_projects_store(store: Dict[str, Any]) -> None:
    _CHAT_PROJECTS_FILE.parent.mkdir(parents=True, exist_ok=True)
    tmp = _CHAT_PROJECTS_FILE.with_suffix(".json.tmp")
    tmp.write_text(json.dumps(store, ensure_ascii=False, indent=2), encoding="utf-8")
    tmp.replace(_CHAT_PROJECTS_FILE)


def _normalise_chat_projects_store(store: Dict[str, Any]) -> Dict[str, Any]:
    ensure_global_app_data()
    projects_in = store.get("projects")
    ...
    if isinstance(projects_in, list):
        for item in projects_in:
            if not isinstance(item, dict):
                continue
            pid = _compact_label(item.get("id"), f"prj_{secrets.token_hex(6)}")
            name = _compact_label(item.get("name"), "Project")
            created = item.get("created_at") or _chat_now()
            updated = item.get("updated_at") or created
            tasks_in = item.get("tasks")
            tasks: List[Dict[str, Any]] = []
            if isinstance(tasks_in, list):
                for task in tasks_in:
                    if not isinstance(task, dict):
                        continue
                    tid = _compact_label(task.get("id"), f"tsk_{secrets.token_hex(6)}")
                    title = _compact_label(task.get("title"), "New Chat")
                    ...
                    task_record = {
                        ...
                        "hermesSessionId": str(
                            task.get("hermesSessionId") or session_id
                        ) if (task.get("hermesSessionId") or session_id) else None,
                        "model_provider": _compact_optional(task.get("model_provider")),
                        "model": _compact_optional(task.get("model")),
                        "created_at": task.get("created_at") or created,
                        "updated_at": task.get("updated_at") or updated,
                    }
                    tasks.append(task_record)
            if not tasks:
                tasks = [_new_chat_task("New Chat")]
            project_record = {
                "id": pid,
                "name": name,
                "workspace_path": _normalise_workspace_path(item.get("workspace_path"), name),
                "created_at": created,
                "updated_at": updated,
                "tasks": tasks,
            }
            if item.get("hermesProfile"):
                project_record["hermesProfile"] = _compact_optional(item.get("hermesProfile"), 80)
            if item.get("hermesProfileWarning"):
                project_record["hermesProfileWarning"] = _compact_optional(
                    item.get("hermesProfileWarning"),
                    300,
                )
            ensure_project_hermes_profile(project_record)
            ensure_project_app_data(project_record)
            for task_record in tasks:
                ensure_task_app_data(project_record, task_record)
            projects.append(project_record)

    if not projects:
        projects = [_new_chat_project("Default Project")]

    current_project_id = store.get("current_project_id")
    if not any(p["id"] == current_project_id for p in projects):
        current_project_id = projects[0]["id"]

    current_task_id = store.get("current_task_id")
    current_project = next(p for p in projects if p["id"] == current_project_id)
    if not any(t["id"] == current_task_id for t in current_project["tasks"]):
        current_task_id = current_project["tasks"][0]["id"]

    return {
        "version": _CHAT_PROJECTS_VERSION,
...
def _load_chat_projects_store() -> Dict[str, Any]:
    store = _normalise_chat_projects_store(_read_chat_projects_store())
    _write_chat_projects_store(store)
    return store


def _find_chat_project(store: Dict[str, Any], project_id: str) -> Dict[str, Any]:
...
def _find_chat_task(project: Dict[str, Any], task_id: str) -> Dict[str, Any]:
...
def _body_fields_set(body: BaseModel) -> set:
    return set(
...
async def get_chat_projects():
    return _load_chat_projects_store()


@app.post("/api/chat/projects")
async def create_chat_project(body: ChatProjectCreate):
    store = _load_chat_projects_store()
    project = _new_chat_project(body.name, body.workspace_path)
    store["projects"].insert(0, project)
    store["current_project_id"] = project["id"]
    store["current_task_id"] = project["tasks"][0]["id"]
    _write_chat_projects_store(store)
    return {"ok": True, "project": project}


@app.patch("/api/chat/projects/{project_id}")
async def update_chat_project(project_id: str, body: ChatProjectPatch):
    store = _load_chat_projects_store()
    project = _find_chat_project(store, project_id)
    fields = _body_fields_set(body)
    if "name" in fields and body.name is not None:
        project["name"] = _compact_label(body.name, project.get("name") or "Project")
    if "workspace_path" in fields:
        project["workspace_path"] = _normalise_workspace_path(
            body.workspace_path,
    ...
    for task in project.get("tasks", []):
        if isinstance(task, dict):
            ensure_task_app_data(project, task)
    project["updated_at"] = _chat_now()
    _write_chat_projects_store(store)
    return {"ok": True, "project": project}


@app.post("/api/chat/projects/{project_id}/tasks")
async def create_chat_task(project_id: str, body: ChatTaskCreate):
    store = _load_chat_projects_store()
    project = _find_chat_project(store, project_id)
    task = _new_chat_task(body.title, body.model_provider, body.model)
    ensure_task_app_data(project, task)
...
@app.patch("/api/chat/projects/{project_id}/tasks/{task_id}")
async def update_chat_task(project_id: str, task_id: str, body: ChatTaskPatch):
    store = _load_chat_projects_store()
    project = _find_chat_project(store, project_id)
    task = _find_chat_task(project, task_id)
    fields = _body_fields_set(body)
    if "title" in fields and body.title is not None:
        task["title"] = _compact_label(body.title, task.get("title") or "New Chat")
    if "session_id" in fields:
        task["session_id"] = str(body.session_id).strip() if body.session_id else None
        task["hermesSessionId"] = task["session_id"]
    if "hermesSessionId" in fields:
        task["hermesSessionId"] = (
            str(body.hermesSessionId).strip() if body.hermesSessionId else None
        )
        task["session_id"] = task["hermesSessionId"]
    if "model_provider" in fields:
        task["model_provider"] = _compact_optional(body.model_provider)
    if "model" in fields:
        task["model"] = _compact_optional(body.model)
    task["updated_at"] = _chat_now()
...
@app.get("/api/chat/projects/{project_id}/tasks/{task_id}/messages")
async def get_chat_task_messages(project_id: str, task_id: str):
    store = _load_chat_projects_store()
    project = _find_chat_project(store, project_id)
    task = _find_chat_task(project, task_id)
    ensure_task_app_data(project, task)
    result = load_task_messages(project_id, task_id)
    _log.debug(
        "redou task messages loaded projectId=%s taskId=%s messagesPath=%s loadedMessages=%s hermesSessionId=%s",
        project_id,
        task_id,
        result.get("messagesPath", ""),
        len(result.get("messages", []) or []),
        result.get("hermesSessionId", ""),
    )
    return result


def _safe_redou_context_file(path_value: Any, project: Dict[str, Any]) -> Path:
    path = Path(str(path_value or "")).expanduser().resolve(strict=False)
    allowed_roots = [get_redou_app_data_root().resolve(strict=False)]
    workspace = str(project.get("path") or project.get("workspace_path") or "").strip()
    if workspace:
        allowed_roots.append((Path(workspace).expanduser().resolve(strict=False) / ".redou"))
    for root in allowed_roots:
        ...
        raise HTTPException(status_code=400, detail="Context file path is outside Redou context roots")
    path.parent.mkdir(parents=True, exist_ok=True)
    return path


def _project_context_file_path(project: Dict[str, Any], kind: str) -> Path:
    ensure_project_app_data(project)
    if kind == "rules":
        return _safe_redou_context_file(project.get("rulesPath"), project)
    raise HTTPException(status_code=404, detail="Unknown project context file")


def _task_context_file_path(project: Dict[str, Any], task: Dict[str, Any], kind: str) -> Path:
    ensure_task_app_data(project, task)
    if kind == "rules":
        return _safe_redou_context_file(task.get("rulesPath"), project)
    if kind == "context":
        return _safe_redou_context_file(task.get("contextPath"), project)
    raise HTTPException(status_code=404, detail="Unknown task context file")


def _read_context_file(path: Path, kind: str) -> Dict[str, Any]:
    try:
...
@app.get("/api/chat/projects/{project_id}/context-files/{kind}")
async def get_project_context_file(project_id: str, kind: str):
    store = _load_chat_projects_store()
    project = _find_chat_project(store, project_id)
    path = _project_context_file_path(project, kind)
    return _read_context_file(path, kind)


@app.put("/api/chat/projects/{project_id}/context-files/{kind}")
async def update_project_context_file(project_id: str, kind: str, body: ChatContextFilePatch):
    store = _load_chat_projects_store()
    project = _find_chat_project(store, project_id)
    path = _project_context_file_path(project, kind)
    ...
    project["updated_at"] = _chat_now()
    _write_chat_projects_store(store)
    return {"ok": True, **_read_context_file(path, kind)}


@app.get("/api/chat/projects/{project_id}/tasks/{task_id}/context-files/{kind}")
async def get_task_context_file(project_id: str, task_id: str, kind: str):
    store = _load_chat_projects_store()
    project = _find_chat_project(store, project_id)
    task = _find_chat_task(project, task_id)
    path = _task_context_file_path(project, task, kind)
    return _read_context_file(path, kind)


@app.put("/api/chat/projects/{project_id}/tasks/{task_id}/context-files/{kind}")
async def update_task_context_file(
    project_id: str,
    task_id: str,
    kind: str,
    body: ChatContextFilePatch,
):
    store = _load_chat_projects_store()
    project = _find_chat_project(store, project_id)
    task = _find_chat_task(project, task_id)
...
def _hydrate_model_setup_catalog(
    catalog: List[Dict[str, Any]],
    cfg: Dict[str, Any],
    current: Dict[str, str],
) -> List[Dict[str, Any]]:
    """Merge saved custom/current model setup back into the curated catalog."""
    ...
    for provider, provider_cfg in user_providers.items():
        ...
        entry["models"] = _dedupe_strings(
            [configured_model, *_provider_model_list(provider_cfg), *(entry.get("models") or [])]
        )
        if configured_model:
...
def _build_model_setup_catalog(cfg: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
    """Curated first-run choices for the model setup dialog.

    The picker still uses /api/model/options for already-authenticated dynamic
    providers. This catalog is intentionally small and focused on popular
    Chinese model providers plus local OpenAI-compatible endpoints.
    """
    try:
        from hermes_cli.auth import PROVIDER_REGISTRY
    ...
    def models(provider: str, fallback: Optional[List[str]] = None) -> List[str]:
        return list(_PROVIDER_MODELS.get(provider) or fallback or [])
    ...
    catalog = [
        ...
        _catalog_entry(
            provider="deepseek",
            label="DeepSeek",
            description="DeepSeek V-series and classic chat/reasoner models.",
            ...
            models=models("deepseek", ["deepseek-v4-pro", "deepseek-v4-flash"]),
            default_model="deepseek-v4-flash",
            region="CN",
            tags=["reasoning", "coding", "1M"],
        ...
        _catalog_entry(
            ...
            label="Kimi / Moonshot",
            description="Moonshot China endpoint for Kimi K2 models.",
        ...
        _catalog_entry(
            ...
            label="Kimi Global",
            description="Moonshot international endpoint for Kimi K2 models.",
            base_url=base("kimi-coding", "https://api.moonshot.ai/v1"),
            api_key_env=key_env("kimi-coding", "KIMI_API_KEY"),
            base_url_env=base_env("kimi-coding", "KIMI_BASE_URL"),
            models=models("kimi-coding", ["kimi-k2.6", "kimi-k2.5", "kimi-k2-thinking"]),
            default_model="kimi-k2.5",
            region="Global",
            tags=["coding", "long-context"],
        ...
        _catalog_entry(
            ...
            models=models("zai", ["glm-5.1", "glm-5", "glm-5-turbo", "glm-4.7"]),
            default_model="glm-5",
            region="CN/Global",
        ...
        _catalog_entry(
            ...
            description="Baidu Qianfan OpenAI-compatible endpoint.",
            ...
            region="CN",
            tags=["ernie", "openai-compatible"],
            docs_url="https://cloud.baidu.com/doc/WENXINWORKSHOP/index.html",
            api_mode="chat_completions",
            custom_provider_name="Baidu ERNIE / Qianfan",
        ),
        _catalog_entry(
            provider="tencent-tokenhub",
            label="Tencent Hunyuan",
            description="Tencent Hunyuan through Tencent MaaS TokenHub.",
            base_url=base("tencent-tokenhub", "https://tokenhub.tencentmaas.com/v1"),
            api_key_env=key_env("tencent-tokenhub", "TOKENHUB_API_KEY"),
            base_url_env=base_env("tencent-tokenhub", "TOKENHUB_BASE_URL"),
            models=models("tencent-tokenhub", ["hy3-preview"]),
    ...
    if isinstance(model_cfg, dict):
        ...
        current = {"provider": "", "model": str(model_cfg) if model_cfg else "", "base_url": ""}
    catalog = _hydrate_model_setup_catalog(catalog, cfg, current)
...
def get_model_setup_catalog():
    try:
        ...
        _log.exception("GET /api/model/setup-catalog failed")
        raise HTTPException(status_code=500, detail="Failed to load model setup catalog")
...
def get_model_options():
    ...
    try:
        ...
        for provider in providers:
            item = dict(provider)
            entry = setup_catalog.get(str(item.get("slug") or ""))
            setup_models = list(entry.get("models") or []) if entry else []
            if setup_models:
...
@app.post("/api/model/setup/refresh")
async def refresh_model_setup_models(body: ModelSetupRefresh):
    """Save provider credentials, then refresh selectable models from /models."""
    provider = (body.provider or "").strip()
    selected_model = (body.model or "").strip()
    base_url = (body.base_url or "").strip().rstrip("/")
    api_key = (body.api_key or "").strip()
    api_key_env = (body.api_key_env or "").strip()
    base_url_env = (body.base_url_env or "").strip()
    api_mode = (body.api_mode or "").strip()
    custom_provider_name = (body.custom_provider_name or "").strip()
    payload_models = _dedupe_strings(body.models or [])
    ...
    try:
        ...
        if custom_provider_name or not is_builtin_provider or payload_models:
            ...
            if payload_models:
                entry["models"] = {m: {} for m in payload_models}
        ...
        _log.exception("POST /api/model/setup/refresh failed")
        raise HTTPException(status_code=500, detail="Failed to refresh model setup")


@app.post("/api/model/setup")
async def setup_main_model(body: ModelSetup):
    """Configure a main model and optional key/endpoint in one dashboard step."""
    provider = (body.provider or "").strip()
    model = (body.model or "").strip()
    base_url = (body.base_url or "").strip().rstrip("/")
    api_key = (body.api_key or "").strip()
    ...
    api_mode = (body.api_mode or "").strip()
    custom_provider_name = (body.custom_provider_name or "").strip()
    models = _dedupe_strings(body.models or [])
    ...
    try:
        ...
        # Non-registry presets are persisted as named custom providers. Runtime
        # resolution can then use model.provider="doubao" (for example) instead
        # of collapsing every OpenAI-compatible endpoint to plain "custom".
        if custom_provider_name or not is_builtin_provider or models:
            providers_cfg = cfg.get("providers")
            if not isinstance(providers_cfg, dict):
                providers_cfg = {}
        ...
        # Prefer .env/key_env for secrets. For anonymous local endpoints, no key
        # is needed. If a user supplies a custom key without an env var, keep it
        # in the model config because runtime custom resolution can read it.
        ...
        _log.exception("POST /api/model/setup failed")
        raise HTTPException(status_code=500, detail="Failed to save model setup")
...
async def delete_skill(body: SkillDelete):
    ...
    try:
        result = json.loads(skill_manage(action="delete", name=name, absorbed_into=""))
    except Exception as exc:
...
# PTY bridge supports POSIX ptyprocess and native Windows pywinpty. If the
# optional dependency is missing, keep the rest of the dashboard available and
# let /api/pty surface a focused install hint.
try:
...
def _resolve_chat_argv(
    ...
    task_resume: Optional[str] = None
    task_model = ""
    task_provider = ""
    hermes_profile = ""
    hermes_profile_home: Optional[Path] = None
    if project_id:
        store = _load_chat_projects_store()
        project = _find_chat_project(store, project_id)
        project_workspace = project.get("workspace_path") or None
        hermes_profile = str(project.get("hermesProfile") or "").strip()
        if hermes_profile and hermes_profile != "default":
            hermes_profile_home = resolve_hermes_profile_home(hermes_profile)
        if task_id:
            task = _find_chat_task(project, task_id)
            task_resume = task.get("hermesSessionId") or task.get("session_id") or None
            task_model = str(task.get("model") or "").strip()
            task_provider = str(task.get("model_provider") or "").strip()
    ...
    env[REDOU_APP_DATA_ENV] = str(get_redou_app_data_root())
    env[REDOU_CHAT_PROJECTS_FILE_ENV] = str(_CHAT_PROJECTS_FILE)
    if project_id:
        env["REDOU_PROJECT_ID"] = project_id
    if task_id:
        env["REDOU_TASK_ID"] = task_id
    if project_workspace:
        Path(project_workspace).mkdir(parents=True, exist_ok=True)
        env["HERMES_CWD"] = project_workspace
        env["TERMINAL_CWD"] = project_workspace
    else:
        env.setdefault("HERMES_CWD", os.getcwd())
    if hermes_profile:
        env["REDOU_HERMES_PROFILE"] = hermes_profile
        env["HERMES_PROFILE"] = hermes_profile
    if hermes_profile_home is not None:
        env["HERMES_HOME"] = str(hermes_profile_home)
    if getattr(sys, "prefix", None) != getattr(sys, "base_prefix", None):
        env.setdefault("VIRTUAL_ENV", sys.prefix)
    if task_model:
        env["HERMES_MODEL"] = task_model
        env["HERMES_INFERENCE_MODEL"] = task_model
    if task_provider:
        env["HERMES_TUI_PROVIDER"] = task_provider
        env["HERMES_INFERENCE_PROVIDER"] = task_provider
    ...
    if resume:
        ...
        if latest_resume:
            ...
            _log.warning(
                "redou chat resume skipped missing session projectId=%s taskId=%s requestedSession=%s",
                project_id or "",
                task_id or "",
                resume,
            )
    ...
    if project_id and task_id:
        ...
        if task_id:
            try:
                task_info = load_task_messages(project_id, task_id)
                messages_path = str(task_info.get("messagesPath") or "")
            except Exception:
                messages_path = ""
        _log.debug(
            "redou chat spawn projectId=%s taskId=%s hermesProfile=%s profileHome=%s appData=%s messagesPath=%s resume=%s",
            project_id,
            task_id,
            hermes_profile or "default",
            str(hermes_profile_home) if hermes_profile_home else "",
            env.get(REDOU_APP_DATA_ENV, ""),
            messages_path,
            env.get("HERMES_TUI_RESUME", ""),
        )
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            "\r\n\x1b[31mChat unavailable: the embedded terminal dependency "
            "is not installed.\x1b[0m\r\n"
            "\x1b[33mInstall it with: pip install -e '.[pty]'\x1b[0m\r\n"
        )
    ...
    task_id = ws.query_params.get("task_id") or None
    ...
    try:
        ...
        await ws.send_text(f"\r\n\x1b[31mChat project error: {exc.detail}\x1b[0m\r\n")
        await ws.close(code=1011)
        return
...
class DashboardLanguageBody(BaseModel):
    language: str
...
async def get_dashboard_language():
    """Return the active dashboard UI language."""
    config = load_config()
    language = _normalise_dashboard_language(
        cfg_get(config, "dashboard", "language", default="zh")
    )
    return {"language": language}


@app.put("/api/dashboard/language")
async def set_dashboard_language(body: DashboardLanguageBody):
    """Set the dashboard UI language (persists to config.yaml)."""
    language = _normalise_dashboard_language(body.language)
    config = load_config()
    if "dashboard" not in config:
        config["dashboard"] = {}
    config["dashboard"]["language"] = language
    save_config(config)
